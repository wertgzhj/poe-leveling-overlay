...
**Status (2026-07-19):** phases **P0–P6 implemented** (P7 auto-tracking is optional and unstarted — needs the GGG OAuth email). Post-P6 follow-ups shipped: in-app **auto-update** + a live version badge, a **visual route/profile editor**, **class-aware gem data filled from poewiki** (821 gems — attributes, level requirements, per-class quest/vendor sources; refresh via the "Fetch gem data" workflow), class **starting gems**, and four playtest-feedback rounds (quest "pick one" choices, new-character detection, cost-ordered buy/reward, colour-blind-safe pips, empty-area click-through). Routes and build profiles are owner-authored files; the Windows release is built in CI. **Parked design proposal:** a one-line-per-gem Gems tab (interactive mock-up reviewed, not built). Feature-by-feature status and the manual test checklist live in [`TESTING.md`](TESTING.md).
...
Backlog (post-v1): vendor recipe hints (movement speed boots, +1 wand), lab layout links, multiple profiles per character, code signing, underleveled/XP-penalty hint (area level from the Generating line vs. character level), poe.ninja/pob link import, one-line-per-gem Gems tab (mock-up reviewed, parked), a "Play/racer" condensed mode. (Auto-update — shipped.)
...
├── NOTICE             third-party attribution (poewiki gem data; see below)
└── LICENSE            MIT
```
...
License: MIT (+ `NOTICE`, which attributes the **poewiki** gem data — the
exile-leveling attribution this originally planned for no longer applies, since
routes became owner-authored and none of its data is used; 2026-07-17). Phase integration: README skeleton ships with P0, each phase updates its affected sections, and the complete user guide is part of the P6 DoD.
...
- **2026-07-28 — pre-release audit: four things that would have embarrassed a public test:** a full pass over the project before handing it to outside testers. (1) **The Siosa fallback had become a liar.** `earliestSource` guessed "Siosa · Act 3" for any gem without its own source — correct while `sources` was empty, wrong for **354 of 820 gems** once the wiki fetch filled the dataset: 54 Vaal, 38 Awakened, 216 Transfigured, plus drop-only supports (Empower/Enlighten/Enhance). The overlay was sending people shopping in Act 3 for gems that only drop. The data already answers the question — the wiki writes an explicit Siosa row for the 391 gems she sells and a Lilly row for 433 — so the guess is suppressed as soon as that stock is in the dataset (`hasBroadVendorData`), and it survives only for a pre-fetch dataset where it's still the best available. A `Template:…` wiki page that had been merged in as a "gem" is gone, and `gemName` now rejects MediaWiki namespaces. (2) **Trial zone matching was prefix-based**, so Act 2's "The Crypt Level 2" matched Act 7's "The Crypt" — a hint for a trial that isn't there, and (completion being zone-first) an Izaro line would have ticked the cruel entry. Exact match now, parenthesised instance suffix still tolerated. (3) **`logLanguage` was a dead setting**: in the schema, in `getSettings()`, in the bridge type, never read — the parser was hardcoded to English. Two of the three tracking lines are localized by the game, so a non-English client parsed zones and never levels, leaving the Gems tab on stage 1 with no explanation. Patterns now resolve through a language registry (only `en` ships — guessing another client's wording would mistrack rather than fail), and `backscan` returns counts per kind so the locale-independent-only signature is detected and reported in the tracker strip. (4) **The Guide tab was the first impression** and its bundled content is 34 placeholder steps; the overlay now opens on **Gems** (works out of the box) and the shipped routes carry `skeleton: true`, so the Guide says they're placeholders and offers the editor — delete the flag from an act once you've written it. Plus: **upcoming quest rewards are no longer dropped by act** (the last place a gem could vanish rather than dim — level gating is the signal we can actually track); `acquisitionsForStage` takes a context object instead of four optional positional arguments; trials re-check the character binding on level-ups (progress no longer loads under `(default)`); PoB link import got a 15s timeout + size cap; `npm audit --omit=dev` is clean. 126 → 132 tests.
- **2026-07-27 — quest order within an act, derived rather than hardcoded (#46) + all-acts audit (#47):** the to-do list now reads in the order you actually play a quest, and the rank is **derived from the data**: a quest's gems scale with where it sits in the campaign, so the *lowest gem level a quest rewards* places it within its act — verified to reproduce the true order for every act with reward data, and it self-updates with each gem refresh. Restricted to quest **rewards**: a vendor's unlock quest would poison the rank ("A Fixture of Fate" unlocks Siosa, who sells level-1 gems, which ranked the game's latest Act 3 quest as its earliest — caught against the real dataset, not a fixture). Audited the engine across all ten acts: quest-reward sources exist only for Acts 1–4 (5/2/3/2 quests — PoE genuinely stops handing out gem rewards after Act 4), so Acts 5–10 run on the vendor path, with Siosa (A3, after the Library) and Lilly Roth (A6, after the first quest) already modelled as class-agnostic broad vendors. The stage nav (◀/▶) moved **above** the acquisition list so a longer or shorter buy list can't move the controls.
- **2026-07-27 — muling, duplicate copies, optional tabs (#45):** three owner ideas. (1) **Mule hint** — a gem another class *begins* with can be had free by rolling a level-1 alt of that class and stashing its two starting gems; the buy row shows "· mule a &lt;class&gt;". Scoped to **starting gems only** (owner correction: muling is about the class's initial skill + support, not quest rewards). (2) **×N copies** — the same gem socketed in two different links means you must own two, counted per stage from the socket groups and badged on the row. (3) **Optional tabs** — Settings → Overlay → *Tabs to show* hides Guide/Gems/Trials individually, with at least one always on (the sanitizer falls back to Guide) and the switcher hidden entirely when only one remains.
- **2026-07-21 — level-gated gem plan (#43) + Izaro trial auto-complete (#44):** the owner reported gems missing from "get these gems" across many stages; root cause was the act-based filter interacting with an over-levelled character (level 15 still in Act 1). Act detection needs a numeric area id and lags reality, so gating moved to the signal we track reliably — PoE's XP **safe zone**, `3 + floor(level/16)` (`safeLevelRange`): a gem whose required level is further above yours is flagged `later`. Per owner instruction it is **dimmed, not hidden** ("Please dim, not hide"), with the level it comes online. Separately, **trials auto-complete from Izaro's voice line**: he speaks a plaque line in `Client.txt` (`] Izaro: …`, verified against a real capture the owner supplied) only as a trial is completed. Six plaque lines cover twelve trials, so the **zone** decides which one and the **act** breaks ties where a zone hosts a trial in two difficulties (The Chamber of Sins Level 2 in Acts 2 and 7); an unknown zone falls back to matching the line. All twelve campaign trials are tracked, grouped per Labyrinth (Normal 6 / Cruel 3 / Merciless 3); cruel/merciless zone names are provisional and the manual toggle covers a miss.
- **2026-07-21 — first release cut, repo public, auto-update naming gotcha:** `v0.1.0` shipped from the GitHub UI (tags can't be pushed from the dev sandbox), the repo went public so the update feed is reachable, and it was renamed `poe-overlay` → `poe-leveling-overlay` (`publish.repo` updated in the same change — otherwise new builds bake a dead feed URL). Then a real bug: the updater **404'd** on v0.2.0 because `productName` has spaces and the two systems sanitize differently — electron-builder writes `latest.yml` with spaces collapsed to **hyphens**, GitHub stores assets with spaces collapsed to **dots**. Fixed with a hyphenated literal `artifactName` for both targets; verified against the live release JSON. `CONTRIBUTING.md` added: how to test a change without touching the live build, cut a release, version it, and the gotchas above.
- **2026-07-19 — DESIGN DECISION: one-line-per-gem Gems tab — proposed, parked (not built):** explored condensing the Gems tab so each gem appears **exactly once** — link-grouped rows, each carrying its socket colour, source verdict (🎁 take / buy · vendor · cost / ✓ start / later), and the "pick one" choice inline — replacing today's split across a reward box, a buy list, and inline link tags (a gem can show in up to three places). An interactive mock-up in the overlay's own tokens was produced and reviewed. **Decision: keep it as a design option for later, don't build it yet** — revisit after more playtesting; the one open question is preserving a clean town "buy" list (likely a buy-filter toggle). README/plan/TESTING refreshed to cover all post-P6 work.
- **2026-07-19 — starting gems (don't buy what you begin with):** each class starts at level 1 with a skill + support gem already in inventory (owner: a Witch starts with Arcane Surge Support), so the overlay was wrongly listing them to buy/quest. New owner-editable `data/starting-gems.json` (class → starter names, not touched by the gem fetch); the engine marks a planned gem that matches the class's starters `starting: true`, drops it from the buy/reward/pick-one lists, and the Gems tab shows a green **"✓ start"** tag on its link row instead. All 7 classes' starters are **owner-confirmed and validated against the wiki gem list** (skill + support each). Engine unit-tested (108).
- **2026-07-19 — playtest round 4: new-character detection, quest-choice grouping, colour/sort polish:** (1) **New character auto-detected** — in auto mode (no explicit character name), entering **The Twilight Strand** (`1_1_1`, only ever reached by a fresh character) releases the adopted binding so the new character's first level-up rebinds; explicit bindings and the frequency-based backscan are untouched (owner: created a new char and the overlay stayed on the old one). (2) **Quest-reward CHOICE made visible** — the Gems tab groups reward gems by the quest that offers them and flags a multi-gem group **"Pick one — take one, buy the rest"** (a quest reward is a single pick in game); act + quest shown per group; choices sort to the top. (3) **Socket pips**: true green (`green-500`) + deep blue (`blue-600`) hues and the **attribute letter inside each pip** (R/G/B) — unambiguous even when green/blue are hard to tell apart (colour-blind-safe). (4) **Sort priority reworked to owner spec**: cost tier → act (1→10) → alphabet (was level-then-name). (5) Act shown in the inline link source tags too. Deferred (owner asked "what do you think?"): condensing the tab to one line per gem. 107 tests.
- **2026-07-19 — playtest round 3: stale-level bug after restart + reward box polish:** the owner reported the Gems tab stuck on the 1–11 stage at level 14 — root cause: the log **backscan replays silently**, so in-main consumers (profile/guide/trials services) that registered before it finished kept stale state until the next LIVE event; the renderer's tracker strip got the snapshot push and looked correct, masking it. `LogService.onBackscan` now pushes the resumed end state (bound level + current area) to all in-main listeners once — the gem stage, guide cursor, and trial hint resume correctly right after an app restart. Plus feedback: **one merged "Quest rewards" box** (current-stage rewards + muted "for later (lvl X+)" rows; accent + "take now" at reward steps) instead of two separate sections; the for-later list is **scoped to the current act** (`actFromAreaId` from numeric area ids; act 3 rewards no longer show while standing in Act 1); rewards/buy lists sort by **gem level requirement then name** (progression order, not alphabet). Cost tiers confirmed correct by the owner. "Play/racer mode" parked as an idea. 103 tests.
- **2026-07-19 — playtest round 2: full gem colours from the wiki + vendor costs:** the hand-curated ~70-gem attribute set kept missing gems the owner actually used (Frostblink, Flame Wall, …), so the fetch now also reads the **`skill_gems`** Cargo table (`primary_attribute` → socket colour, `required_level`) for **every** gem — colour coverage stops being a curation task, and non-gem quest rewards (books) stay excluded because "known gem" now means "in `skill_gems`". Wiki basics win over curated values on merge; curated extras survive. The gem's level requirement drives a **vendor cost tier** on the buy list (Wisdom/Transmutation/Alteration/Chance/Alchemy — `COST_TIERS`, flagged PROVISIONAL, verify in game). 100 tests.
- **2026-07-19 — first Windows playtest feedback round (owner tested P0–P7):** (1) **empty-area click-through fixed** — in interactive mode the transparent window area around the panel swallowed clicks; now the renderer reports hover over visible UI (`data-interactive` surfaces, drag-guarded) and the main process flips `setIgnoreMouseEvents(…, {forward:true})` so only the panel captures the mouse; the version badge is anchored to the panel corner instead of the window. (2) **Trials no longer auto-check on zone entry** (you can walk a zone without doing the trial): entering shows an amber "Trial in this zone" hint on every tab with a one-click *Done ✓*; the trial row shows "you are here". Auto-complete waits on a captured Client.txt completion line (TESTING ask). (3) **Gems tab reworked per feedback:** *Buy (this stage)* moved above the links; every gem line carries an inline source tag (🎁 quest / vendor+act / ≈ fallback); new *"Take when a quest offers it (for later)"* box lists later-stage gems this class gets as quest rewards (engine `upcoming`, tested). (4) **PoB import:** overlapping set titles (the owner's "1-11" + "9-24" → "1–8" case) now clamp *with a warning naming the stage*, and title parsing accepts `Lvl/Lv` shorthand and in-string ranges ("1-11 Freezing Pulse") while ignoring link counts ("4-link setup"). 94 tests.
- **2026-07-19 — P5 gem `sources` FILLED from poewiki (the last P5 data gap closed):** the *Fetch gem data* workflow ran green end-to-end and committed the dataset (guarded by `npm test`): **67/70 curated gems** now carry per-class quest/vendor sources — e.g. Ground Slam (Marauder quest reward A1 + Nessa vendor gated to Duelist/Marauder/Templar), Fireball (Nessa A1 for five classes) — including the wiki's own Siosa/Lilly rows, i.e. the class-gated vendor stock §5.2 demanded. Getting there took a probe-driven debugging loop against a wiki we can't reach from the sandbox (runs #1–#6): `Special:CargoExport` 500s with an empty body → switched to `api.php?action=cargoquery`; unspecific `MWException` → per-column 1-row probes; verdict: **there is no `reward` column** — the Cargo tables attach to each gem's wiki page, so the gem name is `_pageName` (now aliased to `reward`; disambiguation suffixes like "(gem)" stripped). One data-dependent test moved to controlled fixtures. Refreshing after a patch = re-run the workflow (or `npm run fetch-gems` locally).
- **2026-07-19 — version badge with instant update, gem-data fetch as a one-click Action, housekeeping:** (1) the overlay's bottom-right corner is now a **live version badge** (`VersionBadge`): plain `vX.Y.Z` normally, download progress while an update pulls, and a clickable `vOld → vNew ⬆` pill once it's ready — one click installs and restarts immediately (complements the dismissable banner; also flags update-check errors with a tooltip). (2) **"Fetch gem data" workflow** (`fetch-gems.yml`, `workflow_dispatch`): runs `npm run fetch-gems` on an ubuntu runner (full egress — poewiki reachable there, unlike the dev sandbox), guards with `npm test`, and commits the updated `data/gems.json` straight to main; optional *include uncurated* input maps to `--all`. Keeping gem data current is now a button, no local Node needed. (3) Housekeeping: deprecated `actions/checkout@v4`/`setup-node@v4` bumped to v5 in all workflows (Node 20 deprecation warnings); npm deps audited — everything current, the Vite 7 pin stays (electron-vite 5 peers ≤7); sanitized log fixtures are now tracked (`!data/fixtures/*.log`), fixing 6 CI-only test failures; `CLAUDE.md` added with standing workflow rules (incl. **merge PRs on green CI until the repo goes public** — owner's instruction).
- **2026-07-18 — in-app auto-update (electron-updater + GitHub Releases):** the installed build now updates itself, so users never have to check GitHub. A main-process `UpdateService` (`electron/update/service.ts`) wraps electron-updater: on launch (10s in) and every 6h it checks this repo's Releases, **auto-downloads** a newer version in the background, and pushes a small state machine (`checking → downloading{percent} → ready`, or `current`/`error`/`disabled`) to the renderer over new allow-listed IPC (`update:status`/`get`/`check`/`install`). The overlay shows a passive `UpdateBanner` (thin progress while downloading; a prominent **Restart & update** bar when ready — one click = silent per-user reinstall + relaunch) and a full **Settings → Updates** row (current version, manual "Check for updates", the restart button). Build side: a `publish: github` block in `electron-builder.yml` bakes `app-update.yml` in and emits `latest.yml` + blockmaps, and the Windows workflow uploads those to the tagged Release. **Constraint documented:** releases must be publicly downloadable — the repo must be public, or `publish.repo` must point at a public releases repo — else the feed 404s for lack of an embedded token; the app degrades gracefully (an `error`/`disabled` status, no crash, no nag). Disabled entirely in dev/unpackaged (no feed). Typechecks + builds; the update flow itself is a Windows-side check.
- **2026-07-18 — poewiki gem-data fetch script (fills P5 `sources` in one command):** `npm run fetch-gems` reads the Path of Exile Wiki's `quest_rewards` / `vendor_rewards` **Cargo export** (plain JSON, no scraping) and merges per-class quest/vendor sources into `data/gems.json`. Split into a **pure, unit-tested transform** (`scripts/gem-cargo.ts` — row → `sources`, class-list parsing that collapses "all classes" to `undefined`, dedupe + act/quest-before-vendor sort, merge-preserving-attrs) and a thin I/O wrapper (`scripts/fetch-gem-data.ts`) that pages the API and always prints the exact query URLs, so a drifted table/field name surfaces as an inspectable empty result. Defaults to filling sources **only for gems already curated** in `gems.json` (never pulls non-gem quest rewards in); `--all` includes uncurated gems, `--dry-run` previews. This is the tool for the previously-deferred "poewiki data pass" — it's egress-blocked from the build sandbox (403), so the *logic* is verified by tests and the *fetch* is run by the owner on a reachable network. Until run, the Siosa/Lilly fallback still covers sourceless gems. 10 new tests (89 total).
- **2026-07-18 — visual route/profile editor (the deferred "editor window", now built):** a separate **normal window** (electron-vite second renderer entry, `editor/`) with **Routes** and **Profile** tabs, opened from the tray ("Edit routes & profile…") or the overlay Settings. Route editor: per-act step add/edit/delete/reorder with all fields, saved to `userData/routes/actN.json` (overrides the bundled fallback). Profile editor: meta + stages + socket groups (gem-per-line), saved to `userData/profiles/profile.json` and activated (gemPlan preserved as-is). Both **validate through the same validators the app loads files with** (via IPC), so the editor can never write a file the app would reject; saves hot-reload the overlay. This reverses the earlier "editor window dropped in favour of file authoring" calls (§P3/§P4 notes) — files still work, the editor is an addition. Pure edit ops (id-gen, move, serialize) unit-tested; UI is a Windows-side check.
- **2026-07-18 — Siosa/Lilly broad-vendor fallback (P5 §5.2 fallback logic):** `earliestSource` now falls back to the broad vendors — Siosa (Act 3, after "A Fixture of Fate") and Lilly Roth (Act 6+), who sell almost every gem to any class — for a **known** campaign gem that has no gem-specific source yet (unknown names get no fallback, so we never claim Siosa sells a non-gem). So the Gems tab's buy list is useful even before the per-gem `sources` dataset is filled: sourceless gems in the active stage show "· Siosa (Act 3) ≈", with a note that it's a general vendor and may be available earlier as a quest reward. Class-independent and confident, so shipped as real logic (not guessed per-class data). Unit-tested.
